...
//    var friendsListRealm: Results<User>?
...
//        friendsListRealm = realm.objects(User.self)
//
//        token = friendsListRealm?.observe {[weak self] ( changes: RealmCollectionChange) in
//            guard let tableView = self?.friendsTable else {return}
//
//            switch changes {
//
//            case .initial(_):
...
//
//                self?.friendsTable.insertRows(at: insertions.map({ IndexPath(row: $0, section: 0) }), with: .automatic)
//                self?.friendsTable.deleteRows(at: deletions.map({ IndexPath(row: $0, section: 0)}), with: .automatic)
//                self?.friendsTable.reloadRows(at: modifications.map({ IndexPath(row: $0, section: 0) }), with: .automatic)
//
        ...
        if mainUserRealm.first?.friends != nil {
            ...
            apiManager.getFriendsList(token: session.token, id: session.userID){ [weak self] items  in
                ...
                do{
                    ...
                    mainUser?.friends.append(objectsIn: items)
                    
                    for i in 0 ..< ((self?.friendsList.count)!) {
                        self?.friendsList[i].owner = mainUser
                    }
